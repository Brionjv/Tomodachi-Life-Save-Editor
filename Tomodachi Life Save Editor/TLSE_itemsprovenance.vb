...
Imports System.Net
Imports System
Public Class TLSE_itemsprovenance
    ...
    Private Sub Warning_spotpassnint_Click(sender As Object, e As EventArgs) Handles Warning_spotpassnint.Click
        TLSE_dialog.Text_TLSE_dialog.Text = "All Spotpass items in import wear will come from ''Nintendo'' island but Streetpass items in import wear will come from ''Unknow'' island (see issue #31 on Github page)"
        TLSE_dialog.ShowDialog()
    End Sub

    Public Sub Savefileregion()
        If Filever_text.Text = "EU" Or Filever_text.Text = "US" Then
            Spotpassnint = &H18DB00
        ElseIf Filever_text.Text = "JP" Then
            Spotpassnint = &HFA1E0
        ElseIf Filever_text.Text = "KR" Then
            Spotpassnint = &H198EB0
        End If
    End Sub

    Private Sub Check_spotpassnint_CheckedChanged(sender As Object, e As EventArgs) Handles Check_spotpassnint.CheckedChanged
        If Check_spotpassnint.Checked = True Then
            Try
                Dim Writedata As New PackageIO.Writer(savedataArc, PackageIO.Endian.Little)
                Writedata.Position = Spotpassnint
                Writedata.WriteHexString(Text_spotpassnint.Text)
                TLSE_dialog.Text_TLSE_dialog.Text = "Items provenance has been successfully edited"
                TLSE_dialog.ShowDialog()
            Catch ex As Exception
                TLSE_dialog.Text_TLSE_dialog.Text = "Failed to edit Items provenance"
                TLSE_dialog.ShowDialog()
            End Try
        End If
    End Sub
